...
| `normalize_merge_key_format` | bool | `True` | При `True` нормализация ключа merge пробует расширенные fallback-стратегии: `input_format`, затем Google Sheets serial date number (только для колонок `date`/`datetime`). Установите `False` чтобы вернуть поведение до v0.10.0 |
...
Если Sheets хранит дату без применённого date-формата (или конвертирует её обратно в число), API возвращает serial integer (например, `"46023"`). Этот случай также обрабатывается автоматически при `normalize_merge_key_format=True` без дополнительных настроек схемы.
