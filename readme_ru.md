...
| `transient_404_base_delay` | float | `5.0` | Базовая задержка в секундах для экспоненциального backoff между повторами при транзиентном 404 (задержка = `base_delay * 2 ** (attempt - 1)`). Должно быть non-bool real `>= 0`. Худший случай при значениях по умолчанию: до `1 + 3 = 4` полных повторов операции с `5 + 10 + 20 = 35`с backoff между ними — держите Airflow `execution_timeout` с запасом над этим значением |
