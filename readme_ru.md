...
# Добавление строк
# По умолчанию append пишет позиционно (values.update в фиксированный диапазон),
# что делает запись устойчивой к транзиентным 404 без создания дублей.
...
# Сортировка по двум столбцам: сначала новые даты, затем регион А→Я внутри даты
multi_sorted = GoogleSheetsWriteOperator(
    ...
    sort_keys=["date:desc", "region:asc"],   # первичный ключ, затем вторичный
)
```
...
| `append_insert_rows` | bool | `False` | Только для режима `append`. `False` (по умолчанию) пишет добавляемые строки **позиционно** (`values.update` в фиксированный диапазон), что делает запись устойчивой к транзиентным `404` и к ambiguous-success без создания дублей. `True` возвращает легаси-поведение `values.append` (`INSERT_ROWS`) — атомарная вставка строк, но **без** покрытия retry транзиентных 404 (fail-fast на 404). Используйте `True` для раскладок, где под таблицей в тех же столбцах есть контент (футер), который `INSERT_ROWS` должен сдвигать вниз, а не перезаписывать, или когда несколько writer-ов пишут в один лист конкурентно. См. «Поведение append» ниже. Должно быть `bool` |
...
| `transient_404_max_retries` | int | `3` | Сколько раз повторить **всю** операцию, когда Google Sheets возвращает транзиентный `404` на реально существующей таблице (наблюдается после тяжёлых записей). Применяется к идемпотентным режимам — `overwrite` и `merge` — к дефолтному (позиционному) `append`, и к настройке `create_sheet_if_missing`. Легаси-`append` (`append_insert_rows=True`) и read-методы остаются fail-fast. Не-404 `HttpError` пробрасывается сразу. Должно быть non-bool `int >= 0`; `0` отключает retry |
| `transient_404_base_delay` | float | `5.0` | Базовая задержка в секундах для экспоненциального backoff между повторами при транзиентном 404 (задержка = `base_delay * 2 ** (attempt - 1)`). Должно быть non-bool real `>= 0`. Худший случай при значениях по умолчанию: 1 начальный запуск плюс до 3 повторов (4 запуска операции всего) с `5 + 10 + 20 = 35`с backoff между ними — держите Airflow `execution_timeout` с запасом над этим значением |
...
По умолчанию (`append_insert_rows=False`) `append` пишет **позиционно**: один раз читает текущую высоту таблицы, затем пишет новые строки через `values.update` в фиксированный диапазон сразу под ней. Для обычной раскладки (таблица — последний блок контента в своих столбцах, дописываем вниз) видимый результат идентичен легаси-пути `values.append` (`INSERT_ROWS`), поэтому существующие DAG-и получают новое поведение без изменений.

Позиционная запись **идемпотентна на in-process повторе** — повтор записи перезаписывает те же ячейки, а не добавляет новые. Именно это позволяет дефолтному `append` участвовать в retry транзиентных 404 (см. `transient_404_max_retries`) и переживать ambiguous-success внутри hook-level retry без появления дублей.

**Допущения дефолтного пути:**

- **Под таблицей в её столбцах пусто.** Строки непосредственно под последней непустой строкой (в столбцах самой таблицы) считаются пустыми и будут перезаписаны. Если под таблицей в тех же столбцах есть футер или другой контент, установите `append_insert_rows=True`, чтобы `INSERT_ROWS` сдвигал его вниз.
- **Один writer на лист.** Высота фиксируется один раз и переиспользуется при записи; если между чтением и записью другой writer допишет в тот же лист, позиционный путь может перезаписать эти строки (в отличие от атомарного `INSERT_ROWS`). По контракту провайдера в один лист пишет ровно один task/DAG. Для конкурентной атомарной вставки используйте `append_insert_rows=True`.

**Что не покрывается:** позиционный append устойчив только пока таск **жив** (in-process retry 404). Полный **task-retry** Airflow (таск упал и Airflow перезапустил его) не покрывается — свежий прогон перечитает уже выросшую высоту и может дописать повторно. Это ровно то же поведение, что у легаси append; оно не ухудшается — улучшается только in-process случай.

### Наблюдаемость

Каждый режим записи возвращает через XCom result-dict с полем `transient_404_retries` — числом повторов операции после транзиентного `404` (`0`, если их не было). На каждую попытку retry пишется `WARNING`, а при значении больше нуля в конце таска выводится INFO-summary (`"<mode> completed after N transient-404 retries"`).

```python
# например {"mode": "append", "rows_written": 3, "transient_404_retries": 0}
```
...
**Нормализация ключей:** Значения, прочитанные из таблицы на шаге 1, нормализуются к каноническому формату записи из `schema`. При `normalize_merge_key_format=True` (по умолчанию) применяются три стратегии по порядку: (1) парсинг через `format`, (2) парсинг через `input_format`, (3) интерпретация как Google Sheets serial date number (только для колонок `date`/`datetime`). Это обрабатывает региональное переформатирование Sheets, ручное изменение формата ячеек и ячейки с числовым serial-номером. Установите `normalize_merge_key_format=False` чтобы отключить расширенную нормализацию.

**Merge по дате без схемы:** Если для `merge_key` вообще нет записи в `schema`, merge по ключу-дате в ISO-формате (`YYYY-MM-DD`, например `"2026-01-01"`) безопасен "из коробки" при `normalize_merge_key_format=True` (по умолчанию) — колонка автоматически распознаётся как дата по форме входящих значений, а существующий ключ в таблице читается как serial-число, чтобы сопоставление выдерживало смену формата отображения столбца (Дата ↔ Число ↔ другой формат даты). Это автораспознавание понимает только шаблон `YYYY-MM-DD`. Другие форматы дат (например, `"01.03.2026"`) и ключи типа `datetime` (например, `"2026-01-01 12:30:00"`) **не** распознаются без `schema` — для них нужно явно указать `schema` для `merge_key` (с `format`/`input_format` при необходимости, см. ниже). Установите `normalize_merge_key_format=False`, чтобы отключить это распознавание и вернуть строго легаси-поведение (сырые значения сравниваются как есть).
...
Если Sheets хранит дату без применённого date-формата (или конвертирует её обратно в число), API возвращает serial integer (например, `"46023"`). При явной записи `schema` и `type: "date"` этот случай надёжно обрабатывается при `normalize_merge_key_format=True`: колонка ключа читается с render-опцией `SERIAL_NUMBER`, поэтому сопоставление не зависит от формата отображения ячейки. Для ключей типа `date` это также работает **без** какой-либо записи в `schema` вообще, если входящие значения — ISO-даты (`YYYY-MM-DD`) — см. «Merge по дате без схемы» выше. Для `type: "datetime"` колонка ключа всегда читается как `FORMATTED_STRING` (никогда `SERIAL_NUMBER`, чтобы не обрезать время суток в обычном случае); если формат ячейки при этом показывает обычное число, существующий ключ всё равно может быть распознан как serial-число, но только с точностью до даты — **время суток обрезается до полуночи**. Для ключей типа `datetime` всегда требуется явная `schema`; автоматического распознавания без схемы для них нет.
