...
| `table_start` | str | `"A1"` | Верхний левый угол таблицы (например, `"C3"`). Используется в `append` и `merge` для определения строки заголовка и абсолютных позиций колонок. По умолчанию `"A1"` |
