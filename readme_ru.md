...
| `transient_404_max_retries` | int | `3` | Сколько раз повторить **всю** операцию, когда Google Sheets возвращает транзиентный `404` на реально существующей таблице (наблюдается после тяжёлых записей). Применяется только к идемпотентным режимам — `overwrite` и `merge` — и к настройке `create_sheet_if_missing`; `append` и read-методы остаются fail-fast. Не-404 `HttpError` пробрасывается сразу. Должно быть non-bool `int >= 0`; `0` отключает retry |
